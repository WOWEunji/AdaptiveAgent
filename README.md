...
- [AdaptiveAgent](#adaptiveagent)
  ...
  - [부록 D. 인터랙티브 테스트 시나리오](#부록-d-인터랙티브-테스트-시나리오)
...
interactive 모드에서 긴 입력이거나 multi line은  줄 끝에 `\`를 입력하고 Enter로 이어서 작성할 수 있다.
...
`adaptive-agent --workspace ~/my-ws --llm openai test`로 실행되는 6개 시나리오. 각 시나리오는 독립된 임시 workspace에서 순서대로 실행된다.

| ID | 제목 | 검증 목표 |
|:---|:-----|:---------|
| S-01 | 멀티턴 + 스킬 저장 | 멀티턴 대화 → 스킬 저장 전체 흐름 |
| S-02 | 스킬 재사용 확인 | S-01에서 저장한 스킬이 다음 턴에 검색·재사용되는지 |
| S-03 | 잘못된 코드 자가 수정 | 실패한 코드를 Critic이 감지하고 자가 수정 루프로 복구하는지 |
| S-04 | 부족한 데이터 → ask_human → 재시도 | 정보 부족 시 사용자에게 묻고 응답을 받아 재실행하는지 |
| S-05 | 멀티턴 조건 추가 | 이전 턴 결과에 조건을 추가해 재계산하는 멀티턴 맥락 유지 |
| S-06 | 스킬 조회 및 삭제 | skill_list · skill_delete 빌트인 툴 동작 |

**S-01 — 멀티턴 + 스킬 저장**

```
Turn 1: hp >= 100 몬스터 필터링 + 평균 hp 계산 (code_execute)
Turn 2: "지금 스킬 저장해줘" → tool_create → tool_validate → tool_approve 자동
```

코드 생성·실행·비판·요약 전 파이프라인과 스킬 저장 흐름을 한 번에 검증한다.

**S-02 — 스킬 재사용 확인**

S-01과 동일한 workspace를 사용해 저장된 스킬이 Librarian에 의해 자동 검색·재사용되는지 확인한다. 동일 태스크에서 code_execute 대신 저장 스킬이 선택되어야 한다.

**S-03 — 잘못된 코드 자가 수정**

존재하지 않는 도메인 URL로 크롤러 코드를 작성하게 해 의도적인 실행 실패를 유도한다. Critic이 실패를 감지하고 correction LLM이 수정된 코드를 재생성해 통과하는 흐름을 검증한다.

**S-04 — 부족한 데이터 → ask_human → 재시도**

"데이터 정리해줘"처럼 데이터가 없는 태스크를 입력해 Planner가 ask_human을 선택하는지 확인한다. 사용자가 CSV 데이터를 제공하면 이를 바탕으로 정리 코드를 실행한다.

**S-05 — 멀티턴 조건 추가**

1턴에서 평균 score를 계산하고, 2턴에서 "C로 시작하는 사람만 필터"라는 조건을 추가한다. 이전 턴의 데이터와 맥락이 session.history에 유지되어 플래너가 올바른 재계산을 수행하는지 검증한다.

**S-06 — 스킬 조회 및 삭제**

등록된 스킬 전체 목록 조회 후 일괄 삭제. skill_list와 skill_delete 빌트인 툴이 사용자 확인 없이 직접 실행되는지 확인한다.
