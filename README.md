...
AdaptiveAgent는 자연어 작업을 분석해 내장 툴 또는 LLM으로 처리하는 CLI 기반 에이전트 프로젝트입니다. 현재 뼈대는 공유 `AgentState`, `StateMachineRouter`, 역할별 node 계약, 파일 기반 prompt, 툴 레지스트리/샌드박스, `manifest.json` 스킬 카탈로그를 분리해 이후 동적 툴 생성과 self-correction 기능을 확장하기 쉽게 구성했습니다.

## 주요 기능

- CLI 기반 실행 진입점: `python -m adaptive_agent`
- Ollama 기반 기본 LLM 어댑터
- 명시적 CLI 툴 실행(`--tool`)으로 Codespace에서 검증 가능한 내장 툴(`echo`, `list_files`, `file_list`, `file_read`, `file_write`, `file_patch`, `code_execute`, `shell_run`, `test_run`, `ask_human`, `propose_actions`, `tool_create`, `tool_validate`, `tool_approve`, `tool_search`, `memory_read`, `memory_write`, `analyze_requirements`)
- 자연어 task는 rule matching 없이 LLM 계획 JSON으로만 처리
- 하드코딩 대신 `adaptive_agent/prompts/` 아래 파일 기반 영어 지시문 관리
- `StateMachineRouter`와 node 계약 기반으로 Plan/Coder/Critic 흐름 확장 준비
- 검증과 사용자 승인까지 완료된 생성 툴 metadata를 `.adaptive_agent/tools/manifest.json` 스킬 카탈로그에 통합
- 툴 실행 결과 표준화
- 한국어/영어 작업 입력을 고려한 UTF-8 구조
- `unittest` 기반 기본 회귀 테스트
- 외부인도 한눈에 보는 큰 그림 설계 문서: `docs/architecture_blueprint.md`
- `reference.md` 방법론을 반영한 요구사항 분해 문서: `docs/requirements_breakdown.md`
- 프레임워크 독립형 에이전트 기초 설계 문서: `docs/basic_architecture_design.md`
- 큰 아키텍처 결정과 회의록: `docs/architecture_decision_log.md`
...
adaptive_agent/
  ...
  agent.py             # 공개 Agent API와 LLM 계획/정규화 호환 계층
  cli.py               # CLI 파서와 출력 처리
  config.py            # 환경 변수 기반 설정
  router.py            # AgentState 기반 상태 전이 라우터
  llms/
    base.py            # LLMClient 프로토콜
    factory.py         # provider별 LLM 생성
    ollama.py          # Ollama 어댑터
  nodes/
    base.py            # Plan/Coder/Critic Agent 공통 계약
  prompts/
    default/
      plan.txt         # 최초 계획 프롬프트(영어 지시문)
      coder.txt        # 코드 생성/수정 역할 프롬프트
      critic.txt       # 실행 결과 평가/reflection 역할 프롬프트
      correction.txt   # self-correction 프롬프트(영어 지시문)
  skills/
    catalog.py         # manifest.json 기반 스킬 카탈로그
  tools/
...
| Ollama 자연어 실행 | `llm_provider=ollama`, `llm_model=qwen3.5:2b`, `task=AdaptiveAgent를 한 문장으로 설명해줘` |
...
python scripts/aavs_validate.py --provider ollama --model qwen3.5:2b --output-dir aavs-results-ollama
```

Ollama는 GitHub Actions CPU runner에서 모델 pull과 로컬 추론이 느릴 수 있으므로 AAVS 하네스가 기본 scenario timeout을 더 길게 잡는다. 필요하면 `--timeout-seconds 900`처럼 직접 조정할 수 있다.

검증 프롬프트는 영어로 작성되어 있으며, 특정 정답만 맞히도록 유도하지 않고 JSON/CSV 같은 구조화 데이터는 표준 파서를 사용한 실행 가능한 Python 코드로 처리하도록 일반 원칙을 확인한다.

자동 하네스는 AAVS-001, AAVS-003A/B, AAVS-008, AAVS-012, AAVS-016, AAVS-017, AAVS-018을 대상으로 한다. Agent는 툴 실행 후 raw tool output을 반환하므로, 하네스의 `final_response_scope` 필드는 `raw_tool_output`으로 기록된다.
...
- `StateMachineRouter`는 `PlanNode`를 실제 계획 단계로 호출하고, `AgentState.next_node`를 기준으로 `plan`, `execute`, `critique`, `approve`, `store` 같은 전이를 확장할 경계입니다.
- 역할별 노드는 `adaptive_agent/nodes/base.py`의 `AgentNode` / `NodeResult` 계약을 따르며, Plan/Coder/Critic prompt는 `adaptive_agent/prompts/default/` 아래 파일로 관리합니다.
- Skill은 별도 LLM prompt node가 아니라 `adaptive_agent/skills/catalog.py`와 `manifest.json` 기반 metadata 저장 계층입니다.
- LLM 계획 JSON은 기존 `tool`/`respond` 외에도 `use_tool`, `create_tool`, `approve_tool`, `final_answer`를 받아 기존 실행 계약으로 정규화합니다.
...
- `tool_approve`는 사용자 승인 후 검증된 생성 툴 metadata를 `.adaptive_agent/tools/manifest.json`에 등록합니다.
- 생성 툴 metadata의 장기 인덱스는 `.adaptive_agent/tools/manifest.json`이며, `SkillCatalog`가 이름 기준으로 갱신합니다. 검증 전 또는 승인 전 툴은 manifest 검색 후보에 넣지 않습니다.
