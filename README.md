...
- 명시적 CLI 툴 실행(`--tool`)으로 Codespace에서 검증 가능한 내장 툴(`echo`, `list_files`, `file_read`, `file_write`, `code_execute`, `shell_run`, `ask_human`, `propose_actions`, `tool_create`, `tool_search`, `analyze_requirements`)
...
- `code_execute`와 `shell_run`은 별도 프로세스와 임시 작업 디렉터리에서 실행하며 stdout/stderr/exit code/timeout/기대값 판정을 구조화해 반환합니다.
- `ask_human`과 `propose_actions`는 실제 승인을 대신하지 않고 `pending_human_input` 또는 `approval_required` 상태를 반환해 상위 루프가 멈출 수 있게 합니다.
- `tool_create`는 생성 툴 코드를 `.adaptive_agent/tools`에 저장하고 문법 검증까지만 수행합니다. 실제 로딩과 실행 검증은 이후 `tool_validate` 계층에서 확장합니다.
