...
- `StateMachineRouter`와 node 계약 기반으로 Plan/Coder/Critic/Skill Agent 확장 준비
...
adaptive_agent/
  ...
  nodes/
    base.py            # Plan/Coder/Critic/Skill Agent 공통 계약
  prompts/
    default/
...
- `StateMachineRouter`는 `AgentState.next_node`를 기준으로 `plan`, `execute`, `critique`, `approve`, `store` 같은 전이를 확장할 경계입니다.
- 역할별 노드는 `adaptive_agent/nodes/base.py`의 `AgentNode` / `NodeResult` 계약을 따르며, prompt는 `adaptive_agent/prompts/<prompt_set>/` 아래 파일로 관리합니다.
