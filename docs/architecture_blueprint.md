...
| 상태 | 항목 |
| --- | --- |
| 구현됨 | `AgentState`, `StateMachineRouter`(retrieve→plan→{code\|execute\|parallel}→critique 전이, max_steps 가드, unknown_next_node 폴백), `agents/` 5종(Plan/Coder/Executor/Critic/Librarian), Plan/Coder/Critic/Correction prompt 파일, `SkillCatalog`(키워드/태그/카테고리 가중치 Top-K + OpenAI embedding 코사인 유사도 검색·`upsert` 시 embedding 저장, `record_usage`로 usage/failure 통계, `find_stale_entries`로 무결성 감사), `create_embedding_fn()`(OpenAI `text-embedding-3-small`, API key 없으면 None 반환 후 키워드 폴백), 생성 툴 create/validate/approve/search built-in, 승인 후 manifest 등록 + hash mismatch/missing 차단을 거친 generated tool loader, HITL 재개(`agent.resume()` + `SessionStore.save_pending/load_pending/close` + CLI `--resume/--approve/--reject/--input`), Critic verdict→retry/approve/error 라우팅, 정책 차단의 `block_reason` 식별자(`workspace_path`/`sensitive_absolute_path`/`dangerous_shell_pattern`), CoderAgent의 `tool_create` 인자(name/description/code) 누락 검증 + `coder_arguments_invalid` 이벤트, `ExecutorAgent`가 `ExecutorDependencies`로 실행 로직 소유(`_execute_normalized_tool`·`_run_self_correction_loop`·`_run_normalized_plan` + `response.py` 분리), `_run_parallel_plan()`(`{"action":"parallel","actions":[...]}` 플랜을 `ThreadPoolExecutor`로 병렬 실행, `state.parallel_results`에 수집), LibrarianAgent의 catalog 감사·`record_usage` 위임·`generated_tool_usage_recorded` 이벤트, `SessionStore.cleanup()`(TTL/카운트 cap), `artifact_store`·`web_fetch` builtin 실구현, Critic reflection → Plan 재활용 계약 테스트, manifest 재등록 시 stats 보존(`_normalize` + 테스트) |
| 부분 구현 | 없음 |
| 아직 남음 | 없음 |

> 마지막 갱신: 장기 확장 완료(2026-05-02). embedding 기반 Top-K 검색·multi-agent 병렬화 구현 완료. 컨테이너 샌드박스는 범위 제외(subprocess `LocalSandboxBackend` 유지).
